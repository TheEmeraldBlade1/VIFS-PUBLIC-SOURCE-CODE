...
class ShopNode extends FlxSprite
{
    ...
    var portraits2Chars:Array<String> = ['pretender', 'blackparasitep'];

    var pretender:String = "pretender";
    var blackp:String = "blackparasitep";

    function setupPortrait(name:String){
        if(name != 'bf'){
            portrait = new FlxSprite(0, 0);
            if (name == pretender || name == blackp || name == 'bfsusreal' 
                || name == 'henryphone' || name == 'henryplayer' || name == 'pinkplayable' 
                || name == 'yellowp' || name == 'jorsawseep' || name == '1mungus' 
                || name == 'greyp' || name == 'blackdkp'
                || name == 'bf-defeat-normal' || name == 'bf-defeat-scared'
                || name == 'cerbera'|| name == 'clowfoe'
                || name == 'elliep' || name == 'picolobby'
                || name == 'powersp'|| name == 'powers_saxp'
