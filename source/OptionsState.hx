...
class OptionsState extends MusicBeatState
{
	...
	override function create() {
		...
		for (i in 0...options.length)
		{
			...
			//optionText.setGraphicSize(Std.int(500));
	...
	override function update(elapsed:Float) {
		...
		if (controls.ACCEPT) {
			...
			switch(options[curSelected]) {
				...
				case 'Hud':
					openSubState(new HudOptionsSubstate());
				case 'Score Text':
					openSubState(new ScoreTxtOptionsSubstate());
				case 'Notes':
					openSubState(new NoteSplashOptionsSubstate());
				case 'Others':
					openSubState(new SoundOptionsSubstate());
				/*case 'Game':
					openSubState(new GameOptionsSubstate());
				case 'Mods':
					openSubState(new ModOptionsSubstate());*/
				default:
...
class GameplayOptionSubstate extends MusicBeatSubstate
{
	private static var curSelected:Int = 0;
	static var unselectableOptions:Array<String> = [
		'GRAPHICS',
...
class HudOptionsSubstate extends MusicBeatSubstate
{
	private static var curSelected:Int = 0;
	static var unselectableOptions:Array<String> = [
		'GRAPHICS',
			...
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Hide Icon Player 2':
						ClientPrefs.hideiconp2 = !ClientPrefs.hideiconp2;

					case 'Hide Watermark':
						ClientPrefs.watermark = !ClientPrefs.watermark;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Score Zooms':
				daText = "If unchecked, the score text won't zoom on hit";
			case 'Hide HealthBar':
				daText = "If checked, hides the healthbar";
			case 'Acc Rating':
				daText = "If checked, the rating fc will go like MFC, SFC, GFC, FC\nif unchecked the rating fc will go like SFC, GFC, FC, FC";
			case 'Hide Song Length':
				daText = "If checked, the bar showing how much time is left\nwill be hidden.";
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Hide Icon Player 2':
						daValue = ClientPrefs.hideiconp2;
					case 'Hide Watermark':
						daValue = ClientPrefs.watermark;
					case 'Persistent Cached Data':
						daValue = ClientPrefs.imagesPersist;
					case 'Hide Song Length':
						daValue = ClientPrefs.hideTime;
...
class ScoreTxtOptionsSubstate extends MusicBeatSubstate
{
	private static var curSelected:Int = 0;
	static var unselectableOptions:Array<String> = [
		'GRAPHICS',
...
class NoteSplashOptionsSubstate extends MusicBeatSubstate
{
	private static var curSelected:Int = 0;
	static var unselectableOptions:Array<String> = [
		'GRAPHICS',
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Fast Botplay Confirm Anims':
						ClientPrefs.fastBotcaFrames = !ClientPrefs.fastBotcaFrames;
					
					case 'Fast Enemy Confirm Anims':
						ClientPrefs.fastBotcaFrames2 = !ClientPrefs.fastBotcaFrames2;

					case 'Fast Confirm Anims':
						ClientPrefs.fastBotcaFrames23 = !ClientPrefs.fastBotcaFrames23;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Botplay Confirm Anims':
				daText = "If checked, botplay will play an anim on hit";
			case 'Fast Confirm Anims':
				daText = "If checked, confirm anims will be fast";
			case 'Fast Botplay Confirm Anims':
				daText = "If checked, botplay anims will be fast (require BCA)";
			case 'Fast Enemy Confirm Anims':
				daText = "If checked, Enemy Confirm anims will be fast";
			case 'Simple Score':
				daText = "If checked, makes the score rating only show score";
			case 'Fullscreen':
				daText = "obvious isnt it?";
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Botplay Confirm Anims':
						daValue = ClientPrefs.botca;
					case 'Fast Botplay Confirm Anims':
						daValue = ClientPrefs.fastBotcaFrames;
					case 'Fast Enemy Confirm Anims':
						daValue = ClientPrefs.fastBotcaFrames2;
					case 'Fast Confirm Anims':
						daValue = ClientPrefs.fastBotcaFrames23;
					case 'Simple Score':
						daValue = FlxG.save.data.simplescore;
					case 'Circle Notes':
						daValue = FlxG.save.data.circles;
...
class SoundOptionsSubstate extends MusicBeatSubstate
{
	private static var curSelected:Int = 0;
	static var unselectableOptions:Array<String> = [
		'GRAPHICS',
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Force BF Skin':
						ClientPrefs.overRidebfDebug = !ClientPrefs.overRidebfDebug;

					case 'Force GF Skin':
						ClientPrefs.overRidegfDebug = !ClientPrefs.overRidegfDebug;
					
					case 'Force Pet':
						ClientPrefs.overRidepetDebug = !ClientPrefs.overRidepetDebug;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'FPS Counter':
				daText = "If unchecked, hides FPS Counter.";
			case 'Force BF Skin':
				daText = "If checked, bf skin will be forced";
			case 'Force GF Skin':
				daText = "If checked, gf skin will be forced";
			case 'Force Pet':
				daText = "If checked, pet will be forced";
			case 'Low Quality':
				daText = "If checked, disables some background details,\ndecreases loading times and improves performance.";
			case 'Persistent Cached Data':
				daText = "If checked, images loaded will stay in memory\nuntil the game is closed, this increases memory usage,\nbut basically makes reloading times instant.";
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Hold Hp Gain':
						daValue = FlxG.save.data.holdnotehp;
					case 'Force BF Skin':
						daValue = ClientPrefs.overRidebfDebug;
					case 'Force GF Skin':
						daValue = ClientPrefs.overRidegfDebug;
					case 'Force Pet':
						daValue = ClientPrefs.overRidepetDebug;
				}
				checkbox.daValue = daValue;
			}
		}
...
class GameOptionsSubstate extends MusicBeatSubstate
{
	private static var curSelected:Int = 0;
	static var unselectableOptions:Array<String> = [
		'GRAPHICS',
...
class ModOptionsSubstate extends MusicBeatSubstate
{
	private static var curSelected:Int = 0;
	static var unselectableOptions:Array<String> = [
		'GRAPHICS',
